...
<?php
echo "<table width='100%' class='Texto1' ><tr><td >BUSCAR - ELIMINAR POR INTERNISTA</td></tr></table>";
...
if($list_paciente)
{
	$sql_paciente2 = "select * from interno  where rut_int = '$list_paciente'";
	$r_paciente2 = mysql_query($sql_paciente2,$link);
}

$sql_paciente = "select * from interno order by apellido_int";
...
echo "    </td>";
echo "    <td>";
echo "      <input type='submit' class='boton2' name='btn_busca_x_paciente' id='btn_busca_x_paciente' value='BUSCAR POR INTERNISTA' />";
echo "    </td>";
echo "  </tr>";
...
if ($btn_busca_x_paciente or $_SESSION['busca_ficha_pac'] and !$btn_busca_x_fecha)
{
		...
		$sql_paciente = "select * from interno where rut_int = '$rut_pac'";
		$r_paciente = mysql_query($sql_paciente,$link);
		...
		echo "  <tr>";
		echo "    <td colspan='4' class='Texto1'>INTERNISTA";
		echo "    </td>";
		echo "  </tr>";
...
if ($btn_busca_x_paciente or $btn_busca_x_fecha or $btn_del or $btn_kine_ord or $btn_int_ord or $btn_fec_ord or $btn_fec_ord2)
{
	...
	$_SESSION['busca_ficha_pac'] = $rut_pac;
	$sql_ficha = "select * from ficha where rut_int = '$rut_pac' $sort";
	...
	echo "    <td><a href=\"ficha_x_kinesiologo.php\">KINESIOLOGO </a><input type='submit' class='boton4' name='btn_kine_ord'  value='v' /></td>";
	echo "    <td><a href=\"ficha_x_paciente.php\">PACIENTE </a><input type='submit' class='boton4' name='btn_int_ord'  value='v' /></td>";
	echo "    <td>ACCION</td>";
	echo "  </tr>";
	echo "  <tr>";
